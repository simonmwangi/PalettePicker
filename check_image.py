    ...
    if 'image' in response.headers['Content-Type']:
        ...
        if image_type:
            ...
            showinfo(title='Error',
                     message="Error: Unable to verify the signature of the image")
            #exit(1)
    return False
...
def get_image(url):
    try:
        ...
        showinfo(title='Error',
                 message="Error: While requesting url: {0}".format(url))
        # exit(1)
    ...
    if response:
        if check_for_image(response):
            ...
            showinfo(title='Error',
                     message="Sorry: The url doesn't contain any image :(")
